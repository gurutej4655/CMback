...
const showCartItem = async (req, res) => {
  try {
    let data = await Cart.find({ customerEmail: req.params.userEmail });
    for(const element of data){
        let stock = await decreaseProductStocks(element.productId, 0);
        element.stocksLeft = stock
    }
...
const deleteCartItem = async (req, res) => {
  try {
    const data = await Cart.deleteMany({ customerEmail: req.params.userEmail}, { writeConcern: { w: 'majority' } });
    res.status(200).send(data);
  } catch (error) {
